 ...
 * @LastEditTime: 2023-06-05 22:33:07
-->
<template>
  <div id="app" v-cloak>
...
export default {
  ...
  methods: {
    ...
    isFarAndNear(face) {
      ...
      if (this.isFarArr.length > 4) {
        ...
        if (this.Increment(this.isFarArr) || this.Decrease(this.isFarArr)) {
          ...
          const diff = this.GetPercent(first - last, first + last); // 相对偏差 (a - (a + b) / 2) / ((a + b) / 2)
          if (diff <= -5) {
            // this.log(`【动作】靠近`, `info`);
    ...
    isOpenMouth(face, ctx) {
      ...
      const featureIndex1 = [0, 17]; // 17和0是上下嘴唇的特征点
      const featureLocation1 = [];

      const featureIndex2 = [10, 152]; // 152和10是上下脸的特征点
      ...
      // 10,152占0,17的比例
      const proportion = this.GetPercent(this.getDistance( // 上下嘴唇的距离 和 上下脸颊的占比
    ...
    isWink(face, ctx) {
      ...
      const leftEye = [159, 144]; // rightEye 应该是右眼的特征点
      const leftEyeLocation = [];
      const rightEye = [385, 374]; // leftEye 应该是左眼的特征点
